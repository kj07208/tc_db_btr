...
BEGIN
	...
	BEGIN TRY
		BEGIN
			...
			SET @jv_doc_id_old = (SELECT TOP 1 jv_doc_id FROM transfer_activity WHERE btr_key = @btr_key ORDER BY transfer_activity_key)
...
PRINT N'>>> Fetch from Next';  			
				...
				BEGIN
					...
					WHILE @intLOOP < 2
						BEGIN
							...
							EXEC ('BEGIN bzebxfr.p_create_jv(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?); END;', @jv_doc_id OUTPUT, @P_acci_code, @P_coas_code, @P_fund_code, @P_orgn_code, @P_acct_code, @P_prog_code, @P_actv_code, @P_locn_code, @P_dr_cr_ind, @P_line_amt, @P_doc_total_amt, @P_change_type, @P_user_id_in, @P_status_ind OUTPUT, @P_status_message OUTPUT) AT [BANNER-PPRD]	
					...
					IF @P_status_ind = 'C'
						BEGIN
							...
							EXEC ('BEGIN bzebxfr.p_create_jv_dtl(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?); END;', @jv_doc_id OUTPUT, @P_acci_code, @P_coas_code, @P_fund_code, @P_orgn_code, @P_acct_code, @P_prog_code, @P_actv_code, @P_locn_code, @P_dr_cr_ind, @P_line_amt, @P_doc_total_amt, @P_change_type, @P_user_id_in, @P_status_ind OUTPUT, @P_status_message OUTPUT) AT [BANNER-PPRD]	
							IF @P_status_ind = 'C'
								BEGIN
									UPDATE transfer_activity SET jv_doc_id = @jv_doc_id WHERE transfer_activity_key = @transfer_activity_key
								END
							ELSE
								...
								END
							FETCH NEXT FROM transfer_activity_cursor INTO @P_acci_code,@P_coas_code,@P_fund_code,@P_orgn_code,@P_acct_code,@P_prog_code,@P_actv_code,@P_locn_code,@P_dr_cr_ind,@P_line_amt,@P_doc_total_amt,@P_change_type,@P_user_id_in,@transfer_activity_key,@btr_key
						END
			...
			EXEC ('BEGIN bzebxfr.p_complete_jv(?,?,?,?); END;', @jv_doc_id OUTPUT, @P_user_id_in, @P_status_ind OUTPUT, @P_status_message OUTPUT) AT [BANNER-PPRD]
			IF @P_status_ind = 'C'
				BEGIN
